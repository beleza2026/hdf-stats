...
Future<Map<String, String>> _fetchClima(String ciudad) async {
  try {
    final ciudadEnc = Uri.encodeComponent(ciudad.isNotEmpty ? ciudad : 'Buenos Aires');
    final res = await http.get(Uri.parse('https://wttr.in/$ciudadEnc?format=j1')).timeout(const Duration(seconds: 5));
    if (res.statusCode != 200) return {'temp': '--', 'icono': '🌤️'};
    ...
    if (d.contains('sun') || d.contains('clear')) icono = '☀️';
    else if (d.contains('rain') || d.contains('drizzle')) icono = '🌧️';
...
class _MainScreenState extends State<MainScreen> {
  ...
  /// 0 = dashboard, 1 = torneos, 2 = hub Liga — antes de abrir una sección con `_irSeccion`.
  int _parentBeforeSection = 0;
  ...
  Future<void> _inicializarFCM() async {
    ...
    // Suscribir al topic del equipo favorito
    final prefsTopics = await SharedPreferences.getInstance();
    final equipoId = prefsTopics.getInt('equipo_favorito_id');
  ...
  Widget build(BuildContext context) {
    final Widget shell;
    if (_showDashboard) {
      shell = _buildDashboard();
    } else if (_showTorneos) {
      shell = _buildTorneos();
    } else if (_showLiga) {
      shell = _buildLigaHub();
    } else {
  ...
  Future<void> _irSeccion(int index) async {
    ...
    setState(() {
      if (_showDashboard) {
        ...
        _parentBeforeSection = 1;
      } else if (_showLiga) {
        _parentBeforeSection = 2;
      }
  ...
  Widget _buildIndiceMatchgol() {
    return _gatePremium(
      ...
      subtitle: 'Ranking de rendimiento por jugador.',
    );
  }
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _gatePremium(
                  ...
                  title: 'Tabla Moral',
                  subtitle: 'Vista previa y tabla completa.',
                  compact: true,
                ),
              ]),
            ),
    ...
    switch (_selectedIndex) {
      ...
      case 3:
        return _gatePremium(
          _buildArqueros(),
          title: 'Arqueros',
          subtitle: 'Estadísticas de arqueros.',
        );
      ...
      case 6:
        return _gatePremium(
          ...
          subtitle: 'Predicciones HDF™ y votación.',
        );
      ...
      case 11: return _gatePremium(_buildAlFilo(), title: 'Monitor de Bajas', subtitle: 'Al filo de suspensión.');
      case 12: return _gatePremium(_buildExpulsados(), title: 'Monitor de Bajas', subtitle: 'Expulsados y sanciones.');
      case 13:
        return _gatePremium(
          _buildTablaPosesion(),
          title: 'Tabla de posesión',
          subtitle: 'Estadísticas de posesión.',
        );
      case 14:
        return _gatePremium(
          _buildCuerdaFloja(),
          title: 'En la cuerda floja',
          subtitle: 'Equipos en zona de descenso.',
        );
      ...
      case 16: return _gatePremium(_buildRemontadaLigaSeccion(), title: 'Remontada', subtitle: 'Estadística de remontadas LPF.');
      case 17: return _gatePremium(_buildLesionadosSportmonks(), title: 'Monitor de Bajas', subtitle: 'Lesionados LPF.');
      case 18: return const TablaHinchasScreen();
      case 19: return _gatePremium(_tabArbitros(), title: 'Árbitros', subtitle: 'Estadísticas por árbitro.');
          ...
          child: Container(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (Platform.isAndroid) ...[
                ...
                const SizedBox(height: 8),
                const Text('Ingresá tu código para acceder a HDF Stats gratis.', style: TextStyle(color: Colors.white54, fontSize: 13)),
                const SizedBox(height: 12),
                Row(children: [
                  Expanded(
                    child: TextField(
...
// ── LESIONADOS (Sportmonks · LPF) ─────────────────────────────
Widget _buildLesionadosSportmonks() {
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      const Padding(
        ...
        child: Column(
          ...
          children: [
            Row(children: [
              ...
              SizedBox(width: 8),
              Text('LESIONADOS LPF', style: TextStyle(color: Color(0xFFFF5252), fontWeight: FontWeight.bold, fontSize: 13)),
            ]),
            SizedBox(height: 4),
            Text(
              'Fuente Sportmonks · bajas activas por lesión (no suspensiones)',
              style: TextStyle(color: Colors.white38, fontSize: 11),
            ),
          ],
        ),
      ),
      const Expanded(child: SportmonksLesionadosLpfFullPage()),
    ],
  );
}
            ...
            Expanded(child: TabBarView(children: [
              ...
              FutureBuilder<List<dynamic>>(
                ...
                builder: (ctx, snap) {
                  ...
                  // API-Football suele usar "Attacker" en squads; si no normalizamos, esos jugadores no entran en ningún bucket.
                  String bucketPos(String? raw) {
                    final s = (raw ?? '').toLowerCase();
                    if (s.contains('goal')) return 'Goalkeeper';
                    if (s.contains('defend')) return 'Defender';
                    if (s.contains('mid')) return 'Midfielder';
                    if (s.contains('attack') || s.contains('forward') || s.contains('strik') || s.contains('wing')) {
                      return 'Forward';
                    }
                    return 'Other';
                  }
                  final Map<String, List<dynamic>> byPos = {
                    'Goalkeeper': [],
                    'Defender': [],
                    'Midfielder': [],
                    'Forward': [],
                  ...
                  for (final p in players) {
                    byPos[bucketPos(p['position'] as String?)]!.add(p);
                  }
                  final posLabels = {
                    'Goalkeeper': '🧤 ARQUEROS',
                    'Defender': '🛡️ DEFENSORES',
  ...
  Widget _buildTablas() {
    return DefaultTabController(
      ...
      child: Column(children: [
        ...
        Expanded(child: TabBarView(children: [
          _tabPosiciones(),
          _gatePremium(_tabRendimiento('home'), title: 'Tabla local', subtitle: 'Rendimiento como local.', compact: true),
          _gatePremium(_tabRendimiento('away'), title: 'Tabla visitante', subtitle: 'Rendimiento como visitante.', compact: true),
          _gatePremium(_tabUltimos5(), title: 'Últimos 5', subtitle: 'Forma reciente.', compact: true),
          _gatePremium(_tabTiempo('first'), title: '1.er tiempo', subtitle: 'Estadísticas del 1.er tiempo.', compact: true),
          _gatePremium(_tabTiempo('second'), title: '2.do tiempo', subtitle: 'Estadísticas del 2.do tiempo.', compact: true),
          _gatePremium(_buildTablaMoral(), title: 'Tabla Moral', subtitle: 'Tabla moral.', compact: true),
          _gatePremium(_buildCruces(), title: 'Cruces', subtitle: 'Cruces / eliminatorias.', compact: true),
          _gatePremium(_tabAnual(), title: 'Tabla anual', subtitle: 'Acumulado anual.', compact: true),
          _gatePremium(_tabPromedios(), title: 'Tabla de promedios', subtitle: 'Promedios.', compact: true),
          _gatePremium(_tabEquipoDeFecha(), title: 'Equipo de la fecha', subtitle: 'Equipo de la semana.', compact: true),
          _gatePremium(const TablaRachasTab(), title: 'Rachas', subtitle: 'Rachas de equipos.', compact: true),
        ])),
      ]),
    );
  }
  ...
  Widget _tabEquipoDeFecha() {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final fechaNum = figura['_fechaNum'] as int? ?? 0;
        final fechaLabel = fechaNum > 0 ? 'FECHA $fechaNum' : 'ÚLTIMA FECHA';
            ...
            child: Column(children: [
              Padding(padding: const EdgeInsets.only(top: 10, bottom: 2),
                child: Text(fechaLabel,
                    style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold, fontSize: 11, letterSpacing: 1.5))),
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: [
          Container(
            ...
            child: Text(
              'Solo técnicos en actividad (últimos ${ApiService.dtVentanaActividadDias} días). Racha = Liga + copas con equipos argentinos. Alertas: 5+ sin ganar o 3+ derrotas seguidas.',
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        _dtHeaderCol('%'),
                        _dtHeaderCol('Racha'),
                        const SizedBox(width: 88, child: Center(child: Text('Últ. 5', style: TextStyle(color: Colors.white38, fontSize: 9, fontWeight: FontWeight.bold)))),
                      ],
                    ),
                  ),
                ),
          ...
          Container(
            ...
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Text('Tocá un técnico para ver su carrera completa', style: TextStyle(color: Colors.white24, fontSize: 10)),
            ]),
          ),
        ]);
      },
    ...
    return RepaintBoundary(
      ...
      child: Container(
        ...
        child: Column(children: [
          Row(
            ...
            Container(
              ...
              child: (foto == null || foto.isEmpty)
                  ? const Icon(Icons.person, color: Color(0xFF00C853), size: 16)
                  : null,
            ),
            const SizedBox(width: 6),
            Expanded(
                          ...
                          .map(
                            (r) => Container(
                              ...
                              child: Center(
                                child: Text(
                                  r,
                                  style: TextStyle(color: resultColor(r), fontSize: 6.5, fontWeight: FontWeight.bold),
                                ),
                              ),
                            ),
                          )
          ...
          if (sin5 || tresL)
            Padding(
              padding: const EdgeInsets.only(left: 62, top: 6),
              child: Wrap(
                spacing: 6,
                runSpacing: 4,
                children: [
          ...
          if (presionLabel.isNotEmpty) ...[
            const SizedBox(height: 8),
            Padding(
              padding: const EdgeInsets.only(left: 62, right: 4),
...
/// Elimina caracteres fuera del BMP (emoji de banderas, etc.) que muestran □ en algunos Android.
String _cleanPlayerName(String name) {
  return String.fromCharCodes(
    name.runes.where((r) => r <= 0xFFFF),
  ).trim();
}
...
Widget _tabTiempo(String tipo) {
  ...
  Widget _buildEquipos() {
    return DefaultTabController(
      ...
      child: Column(children: [
        ...
        Expanded(child: TabBarView(children: [
          _tabGoleadores(),
          _gatePremium(_tabAsistencias(), title: 'Asistencias', subtitle: 'Tabla de asistencias.', compact: true),
          _gatePremium(_tabEficacia(), title: 'Eficiencia goleadora', subtitle: 'Eficiencia de delanteros.', compact: true),
        ])),
      ]),
    );
  }
  ...
  Widget _buildArqueros() {
    return FutureBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        final porRachaLiga = List<Map<String, dynamic>>.from(todos);
        porRachaLiga.sort((a, b) {
          final rA = (a['rachaMinSinGolLiga'] as num?)?.toInt() ?? 0;
          final rB = (b['rachaMinSinGolLiga'] as num?)?.toInt() ?? 0;
          if (rB != rA) return rB.compareTo(rA);
          final cA = (a['statistics'][0]['goals']?['conceded'] as num?)?.toInt() ?? 999;
          final cB = (b['statistics'][0]['goals']?['conceded'] as num?)?.toInt() ?? 999;
        ...
        return DefaultTabController(
          ...
          child: Column(children: [
            ...
            Expanded(child: TabBarView(children: [
              ...
              ListView(padding: const EdgeInsets.all(16), children: [
                _sectionTitle('RACHA EN LIGA (MIN SIN RECIBIR GOL)'),
                const SizedBox(height: 4),
                const Text(
                  'Ventana actual del arquero; usa eventos + fixture. Puede ser 0 si faltan datos.',
                  style: TextStyle(color: Colors.white38, fontSize: 10, height: 1.3),
                ),
                const SizedBox(height: 12),
                ...porRachaLiga.asMap().entries.map((e) {
                  final a = e.value;
                  final stats = a['statistics'][0];
                  final player = a['player'];
                  final racha = (a['rachaMinSinGolLiga'] as num?)?.toInt() ?? 0;
                  final conc = stats['goals']?['conceded'] as int? ?? 0;
                  final apps = stats['games']?['appearences'] as int? ?? 0;
                  return _arqueroCard(
                    (e.key + 1).toString(),
                          ...
                          Container(
                            ...
                            child: Row(children: [
                              ...
                              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                ...
                                Text(_cleanPlayerName(figura.key), style: const TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.bold)),
    ...
    return FutureBuilder<Map<String, dynamic>>(
      ...
      builder: (ctx, cSnap) {
        ...
        return Container(
          margin: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
          padding: const EdgeInsets.all(12),
          decoration: BoxDecoration(color: const Color(0xFF1B2A3B), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white12)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (tieneClima) ...[
              ...
              const SizedBox(height: 8),
              Row(children: [
                Text('${ApiService.climaEmoji(desc)}  ${temp}°C', style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
                const SizedBox(width: 8),
  ...
  Widget _h2hCard(String local, String visitante, int homeId, int awayId) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: ApiService.getHeadToHead(homeId, awayId),
      builder: (ctx, snap) {
        if (!snap.hasData || snap.data!.isEmpty) return const SizedBox();
        final h2h = snap.data!;

        int gL = 0, gV = 0, gE = 0;
        int cL_g = 0, cL_e = 0, cL_p = 0;
        int cV_g = 0, cV_e = 0, cV_p = 0;

        for (final f in h2h) {
          final hg = f['goals']?['home'] as int? ?? 0;
          final ag = f['goals']?['away'] as int? ?? 0;
          final esLocal = f['teams']?['home']?['id'] == homeId;
          final gano   = esLocal ? hg > ag : ag > hg;
          final empato = hg == ag;
          if (gano)        { gL++; esLocal ? cL_g++ : cV_g++; }
          else if (empato) { gE++; esLocal ? cL_e++ : cV_e++; }
          else             { gV++; esLocal ? cL_p++ : cV_p++; }
        }

        final totalLocal = cL_g + cL_e + cL_p;
        final totalVisit = cV_g + cV_e + cV_p;
        final localNombre = local.split(' ').first;
        ...
        Widget resumenFila(String label, int g, int e, int p) {
          final total = g + e + p;
          ...
          return Padding(
            ...
            child: Row(children: [
              ...
              const Spacer(),
              chip('$g G', const Color(0xFF00C853)),
              const SizedBox(width: 4),
              chip('$e E', Colors.white38),
              const SizedBox(width: 4),
              chip('$p P', Colors.red),
              const SizedBox(width: 6),
              Text('($total)', style: const TextStyle(color: Colors.white24, fontSize: 10)),
            ]),
          );
        }
        ...
        return Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              const Text('HISTORIAL H2H', style: TextStyle(color: Color(0xFF00C853), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
              const Spacer(),
              Text('Últ. ${h2h.length} partidos', style: const TextStyle(color: Colors.white38, fontSize: 10)),
            ]),
            const SizedBox(height: 10),
            Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
              Column(children: [
                Text('$gL', style: const TextStyle(color: Color(0xFF00C853), fontSize: 22, fontWeight: FontWeight.bold)),
                Text(localNombre, style: const TextStyle(color: Colors.white54, fontSize: 10)),
              ]),
              Column(children: [
                Text('$gE', style: const TextStyle(color: Colors.white38, fontSize: 22, fontWeight: FontWeight.bold)),
                const Text('Empates', style: TextStyle(color: Colors.white38, fontSize: 10)),
              ]),
              Column(children: [
                Text('$gV', style: const TextStyle(color: Color(0xFF1E88E5), fontSize: 22, fontWeight: FontWeight.bold)),
                Text(visitante.split(' ').first, style: const TextStyle(color: Colors.white54, fontSize: 10)),
              ]),
            ]),
            const SizedBox(height: 8),
            if (gL + gE + gV > 0) ClipRRect(
              borderRadius: BorderRadius.circular(4),
              child: Row(children: [
                if (gL > 0) Expanded(flex: gL, child: Container(height: 5, color: const Color(0xFF00C853))),
                if (gE > 0) Expanded(flex: gE, child: Container(height: 5, color: Colors.white24)),
                if (gV > 0) Expanded(flex: gV, child: Container(height: 5, color: const Color(0xFF1E88E5))),
              ]),
            ),
            if (totalLocal > 0 || totalVisit > 0) ...[
              const SizedBox(height: 10),
              const Divider(color: Colors.white10, height: 1),
              const SizedBox(height: 6),
              if (totalLocal > 0) resumenFila('$localNombre como local', cL_g, cL_e, cL_p),
              if (totalVisit > 0) resumenFila('$localNombre de visita', cV_g, cV_e, cV_p),
            ],
            const SizedBox(height: 10),
            const Divider(color: Colors.white10, height: 1),
            const SizedBox(height: 6),
            const Text('Últimos enfrentamientos', style: TextStyle(color: Colors.white38, fontSize: 10)),
            const SizedBox(height: 6),
            ...h2h.take(10).map((f) {
              final hg = f['goals']?['home'] as int? ?? 0;
              final ag = f['goals']?['away'] as int? ?? 0;
              ...
              final fecha = (f['fixture']?['date'] as String? ?? '').length >= 10
                  ? (f['fixture']['date'] as String).substring(0, 10) : '';
              final esLocal = f['teams']?['home']?['id'] == homeId;
              Color rc = Colors.white24;
              if ((hg > ag && esLocal) || (ag > hg && !esLocal)) rc = const Color(0xFF00C853);
              else if ((ag > hg && esLocal) || (hg > ag && !esLocal)) rc = Colors.red;
              return Padding(
                padding: const EdgeInsets.symmetric(vertical: 2),
                child: Row(children: [
                  SizedBox(width: 76, child: Text(fecha, style: const TextStyle(color: Colors.white38, fontSize: 10))),
                  Expanded(child: Text(hn, style: const TextStyle(color: Colors.white60, fontSize: 11), overflow: TextOverflow.ellipsis, textAlign: TextAlign.right)),
                  Container(
                    margin: const EdgeInsets.symmetric(horizontal: 6),
                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                    decoration: BoxDecoration(color: rc.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(4)),
                    child: Text('$hg - $ag', style: TextStyle(color: rc, fontSize: 11, fontWeight: FontWeight.bold)),
                  ),
                  Expanded(child: Text(an, style: const TextStyle(color: Colors.white60, fontSize: 11), overflow: TextOverflow.ellipsis)),
                ]),
              );
            }),
          ]),
  ...
  Widget _tablaRealMoralCard(String local, int homeId, String visitante, int awayId) {
    return FutureBuilder<Map<String, List<Map<String, dynamic>>>>(
      future: ApiService.getTablaMoral(),
      builder: (ctx, snap) {
        if (!snap.hasData) return const SizedBox();
        Map<String, dynamic>? hm, am;
        for (final zona in snap.data!.values) {
          for (final eq in zona) {
            if (eq['id'].toString() == homeId.toString()) hm = eq;
            if (eq['id'].toString() == awayId.toString()) am = eq;
          }
        }
        if (hm == null && am == null) return const SizedBox();

        Widget row(String nombre, Map<String, dynamic>? m) {
          final pts = m?['pts'] as int? ?? 0;
          final pj  = m?['pj'] as int? ?? 0;
          final ptsReal = m?['ptsReal'] as int? ?? 0;
          final diff = pts - ptsReal;
          final diffStr = diff > 0 ? '+$diff' : '$diff';
          final diffColor = diff > 0 ? const Color(0xFF00C853) : diff < 0 ? Colors.red : Colors.white38;
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 6),
            child: Row(children: [
              Expanded(child: Text(nombre, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis)),
              Text('$pj PJ  ', style: const TextStyle(color: Colors.white38, fontSize: 10)),
              Text('$ptsReal real  ', style: const TextStyle(color: Colors.white54, fontSize: 11)),
              Text('$pts moral', style: const TextStyle(color: Color(0xFFFFD700), fontSize: 11, fontWeight: FontWeight.bold)),
              Text('  $diffStr', style: TextStyle(color: diffColor, fontSize: 11, fontWeight: FontWeight.bold)),
            ]),
          );
        }
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFF1B2A3B), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white12)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('TABLA REAL VS MORAL', style: TextStyle(color: Color(0xFF00C853), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.5)),
            const SizedBox(height: 6),
            if (hm != null) row(local, hm),
            if (hm != null && am != null) const Divider(color: Colors.white10, height: 1),
            if (am != null) row(visitante, am),
          ]),
        );
      },
    );
  ...
  Widget _previewEquipoWidget(String equipo, Map<String, dynamic> preview) {
    final topRating = List<Map<String, dynamic>>.from(preview['rating'] ?? []);
    final topGoles  = List<Map<String, dynamic>>.from(preview['goles'] ?? []);
    final promedioEdad = (preview['promedioEdad'] as num?)?.toDouble() ?? 0.0;
    if (topRating.isEmpty && topGoles.isEmpty) return const SizedBox();

    Widget jugadorRow(Map<String, dynamic> p, {bool showGoles = false}) {
      final foto    = p['foto'] as String? ?? '';
      final nombre  = _cleanPlayerName(p['nombre'] as String? ?? '');
      final rating  = (p['rating'] as double?)?.toStringAsFixed(1) ?? '';
      final goles   = p['goles'] as int? ?? 0;
      final edad    = p['edad'] as int? ?? 0;
      return Padding(
        ...
        child: Row(children: [
          CircleAvatar(
            ...
            backgroundColor: const Color(0xFF1B2A3B),
            backgroundImage: foto.isNotEmpty ? NetworkImage(foto) : null,
            child: foto.isEmpty ? const Icon(Icons.person, color: Colors.white38, size: 14) : null,
          ),
          const SizedBox(width: 8),
          Expanded(child: Text(nombre, style: const TextStyle(color: Colors.white, fontSize: 12), overflow: TextOverflow.ellipsis)),
          if (edad > 0)
            Padding(
              padding: const EdgeInsets.only(right: 6),
              child: Text('$edad', style: const TextStyle(color: Colors.white38, fontSize: 10)),
            ),
          if (showGoles)
            Container(
              ...
              child: Text('⚽ $goles', style: const TextStyle(color: Color(0xFF00C853), fontSize: 11, fontWeight: FontWeight.bold)),
            )
          else
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2),
              decoration: BoxDecoration(color: const Color(0xFFFFD700).withValues(alpha: 0.15), borderRadius: BorderRadius.circular(4)),
              child: Text('★ $rating', style: const TextStyle(color: Color(0xFFFFD700), fontSize: 11, fontWeight: FontWeight.bold)),
            ),
        ]),
      );
    }
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text(equipo.toUpperCase(), style: const TextStyle(color: Color(0xFF00C853), fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
          const Spacer(),
          if (promedioEdad > 0)
            Text('Edad prom: ${promedioEdad.toStringAsFixed(1)}', style: const TextStyle(color: Colors.white38, fontSize: 10)),
        ]),
        const SizedBox(height: 8),
        if (topRating.isNotEmpty) ...[
          const Text('Top Rating', style: TextStyle(color: Colors.white38, fontSize: 10)),
          const SizedBox(height: 4),
          ...topRating.map((p) => jugadorRow(p)),
        ],
        if (topGoles.isNotEmpty) ...[
          const SizedBox(height: 8),
          const Text('Goleadores', style: TextStyle(color: Colors.white38, fontSize: 10)),
          const SizedBox(height: 4),
          ...topGoles.map((p) => jugadorRow(p, showGoles: true)),
        ],
      ]),
    );
  }
...
class _PlantelTabState extends State<_PlantelTab> {
  ...
  Widget build(BuildContext context) {
    ...
    porPosicion.forEach((pos, lista) {
      ...
      for (final j in lista) {
        final p = j['player'] as Map<String, dynamic>? ?? {};
        final stats = (j['statistics'] as List?)?.first as Map<String, dynamic>? ?? {};
        final games = stats['games'] as Map<String, dynamic>?;
        final nombre = p['name'] as String? ?? '';
        final foto = p['photo'] as String?;
        final edad = p['age'];
        final nacionalidad = (p['nationality'] as String?)?.trim() ?? '';
        final flag = flagEmojiFromCountryName(nacionalidad);
        final numRaw = games?['number'];
        final dorsal = numRaw is int
            ? numRaw
            : int.tryParse('$numRaw') ??
                (p['number'] is int ? p['number'] as int : int.tryParse('${p['number'] ?? ''}')) ??
                0;
        final partidos =
            (games?['appearences'] as num?)?.toInt() ?? (games?['appearances'] as num?)?.toInt() ?? 0;
        final goles = (stats['goals'] as Map?)?['total'] as int? ?? 0;
        final asist = (stats['goals'] as Map?)?['assists'] as int? ?? 0;
        final ratingStr = games?['rating']?.toString() ?? '0';
        final rating = double.tryParse(ratingStr) ?? 0.0;
        final playerId = (p['id'] as num?)?.toInt() ?? 0;
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 2),
                        const Text('rating torneo', style: TextStyle(color: Colors.white38, fontSize: 9)),
                        const SizedBox(height: 4),
                        Text(
                          '$partidos PJ',
                ...
                children: [
                  if (playerId <= 0)
                    const Padding(
                      ...
                      child: Text('Sin id de jugador', style: TextStyle(color: Colors.white38, fontSize: 11)),
                    )
                  else
                    FutureBuilder<Map<String, dynamic>>(
                      ...
                      builder: (context, snap) {
                        ...
                        final d = snap.data!;
                        final pjC = d['pjClub'] as int? ?? 0;
                        final gC = d['golesClub'] as int? ?? 0;
                        final pjT = d['pjCarrera'] as int? ?? 0;
                        final gT = d['golesCarrera'] as int? ?? 0;
                        final muestras = d['temporadasMuestra'] as int? ?? 0;
                        final tot = d['temporadasTotal'] as int? ?? 0;
                        return Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text(
                            ...
                            const SizedBox(height: 6),
                            Text(
                              'En este club (suma consultada): $pjC PJ · $gC goles',
                            ...
                            Align(
                              ...
                              child: TextButton(
                                onPressed: () => showPlayerCareerSheet(
                                  context,
                                  playerId: playerId,
                                  clubTeamId: widget.teamId,
                                  playerName: nombre,
                                ),
                                child: const Text('Ficha completa', style: TextStyle(color: Color(0xFF00C853))),
                              ),
                            ),
                          ],
                        );
